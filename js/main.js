    ...
    (entries) => {
      entries.forEach((entry) => {
        if (
          ...
          // Extract number, prefix, and suffix from text
          const match = value.match(/^([^\d]*)(\d+)(.*)$/);
          if (match) {
            const prefix = match[1]; // Everything before the number (like "& ")
            const number = parseInt(match[2]); // The number itself
            const suffix = match[3]; // Everything after the number (like "/7")

            target.classList.add("animated");
            animateNumber(target, number, 2000, prefix, suffix);
          }
        }
      });
    },
